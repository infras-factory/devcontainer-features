...
echo -e "${PURPLE}========================================${NC}"
echo -e "${PURPLE}  PYTHON VERSION MANAGEMENT (PYENV) DEBIAN TESTS${NC}"
echo -e "${PURPLE}========================================${NC}"
...
# Check required dependencies for building Python
...
check "pyenv works on Debian" bash -c "export PYENV_ROOT='$USER_HOME/.pyenv' && export PATH='$PYENV_ROOT/bin:$PATH' && pyenv --version"
...
# Check that pyenv doctor can run (it will check for missing dependencies)
check "pyenv doctor runs" bash -c "export PYENV_ROOT='$USER_HOME/.pyenv' && export PATH='$PYENV_ROOT/bin:$PATH' && pyenv doctor &>/dev/null || true"
